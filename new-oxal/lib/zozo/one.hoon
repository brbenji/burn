...
++  req
  ...
  ++  is-datastar
    ^-  ?
    .=  `'true'
    (get-header:http 'datastar-request' header-list.http-req)
  ::
  ++  route
    ^-  pith
    :*
        ...
        ==
      ::
        f+=(our.quest src.quest)
      ::
        `@tas`(~(gut by pams.http-req) 'action' '')
      ::
        stem.http-req
    ==
  ::
  ++  signals
    ...
    %-  need
    %-  de:json:html
    ?^  j=(~(get by pams.http-req) 'datastar')  u.j
    ?~  b=body.http-req  ''
    +:u.b
    ::
  ::
  ++  multipart-form-encoded-body
    ^-  (map @t part:multipart)
    %-  malt
    %+  fall
      %+  de-request:multipart
        header-list.http-req
      body.http-req
    ~
  ::
  ++  content-type-mite
  ...
  ++  form-encoded-body
    ...
    %-  fall  :_  ~
    %-  mole  |.
    (malt (rash +:(need body.http-req) yquy:de-purl:html))
  ::
  ++  payload-cards
    |=  pl=simple-payload:http
    ^-  (list card-outer)
    (^payload-cards rid.http-req pl)
  ::
  ::  response cards
  ::
  ++  feather-1           (corl payload-cards feather-1-payload)
  ++  simple-html         (corl payload-cards simple-html-payload)
  ++  static-html         (corl payload-cards static-html-payload)
  ++  static-mime         (corl payload-cards static-mime-payload)
  ++  static-resource     (corl payload-cards static-resource-payload)
  ++  redirect            (corl payload-cards redirect-payload)
  ++  datastar-response   (corl payload-cards datastar-response-payload)
  ++  empty-response
    %+  payload-cards
      :-  200  ~
    ~
  ++  empty-datastar-response
    (datastar-response ~ ~)
  ::
  ::  responses payloads
  ::
  ::
  ++  datastar-response-payload
    |=  [sig-list=(list (pair @t @t)) fragments=(list datastar-fragment)]
    %^  static-resource-payload  ~  'text/event-stream'
